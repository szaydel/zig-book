...
For our first small project we are going to build a base64 encoder/decoder.
Base64 is an encoding system designed to represent binary data (such as files,
images, or binary executables) as ASCII strings. This encoding is widely used
to encode data for transmission over text-based systems such as email, JSON, or
XML, where binary data might cause issues or not be supported.

A common modern use case for base64 encoding is email, particularly with
services like Gmail and Outlook. Email systems typically rely on the Simple
Mail Transfer Protocol (SMTP), which only supports text data. Therefore, binary
files such as images, photos, or PDFs must be encoded into a textual format,
usually base64, before being attached to an email.

Notably, base64 can encode any byte sequence, as it imposes no restrictions on
the input data. Whether it’s a binary file or an ASCII string, the algorithm
treats all inputs as byte sequences without any inherent meaning.



## How the base64 algorithm works?

Let's first discuss how the base64 algorithm works. We are going to learn about
the base64 scale, which is the 64-character alphabet that is the basis for the
base64 encoding system.

After that, I explain the algorithm behind the base64 encoder, which is
responsible for encoding a given byte sequence into a base64 string. Next, I
explain the algorithm behind the base64 decoder, which is responsible for
decoding base64 messages back into their original meaning.
...
So, in order to convert any sequence of bytes to a base64 encoded string, we need to convert each binary number to the corresponding character in the base64 alphabet.

The base64 alphabet starts with all ASCII uppercase letters (A to Z) which represents
...
These are the 64 characters that compose the base64 scale. The equal sign
character (`=`) is not part of the alphabet, but it is a special character in
the base64 encoding system. This character is used solely as a suffix, to mark
the end of the character sequence, or, to mark the end of meaningful characters
in the sequence.
...
The most efficient way to represent this scale in code is by using a 
*lookup table*. Lookup tables are a common strategy in computer science to 
optimize performance. They work by replacing time-consuming runtime 
calculations with simple array indexing operations, significantly speeding up 
computations.

Instead of performing calculations every time a result is needed, you can 
precompute all possible results at once and store them in an array (acting like
 a "table"). Whenever you need a character from the Base64 scale, you simply 
retrieve it from this array. This approach eliminates the need for resource-
intensive calculations by directly accessing the precomputed value from memory.
...
The algorithm behind a base64 encoder usually works on a 3 byte window. Because
each byte has 8 bits, 3 bytes forms a set of $8 \times 3 = 24$ bits. This is 
desirable for the base64 algorithm, because 24 bits is evenly divisible by 6, 
which forms $24 / 6 = 4$ groups of 6 bits each.

The base64 algorithm operates by converting 3 bytes of data at a time into 4 
characters from the base64 character set. It processes the input string in 
chunks of 3 bytes, converting each chunk into 4 base64 characters per 
iteration. This process continues until the algorithm reaches the end of the 
input string.

Now, you may think, what if you have a particular string that has a number of 
bytes that is not divisible by 3? What happens? For example, if you have a 
string that contains only two characters/bytes, such as "Hi". How does the 
algorithm behave in this situation? You find the answer in @fig-base64-algo1. 
